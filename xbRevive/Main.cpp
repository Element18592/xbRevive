...
//Jasper 200 Pages 400 Blocks 4KB block 0x1000000  0x000231D4 

//Corona 0x00043000

//Xenon 0x01198010 16MB

//Falcon 0x01198010 16MB

//Zepher 0x01198030 16MB

//Jasper 16 MB NAND WITH CONFIG 0x00023010 

//Jasper 256 MB NAND WITH CONFIG 0x008A3020 

//Jasper 512 MB NAND WITH CONFIG 0x00AA3020 
...
void PrintStatus(DWORD Status)
{
	...
	BitMaskThing Things[] = {
		...
		{ "STATUS_WR_ER", STATUS_WR_ER },
		{ "STATUS_ECC_ER", STATUS_ECC_ER },
		{ "STATUS_RNP_ER", STATUS_RNP_ER },
		{ "STATUS_BB_ER", STATUS_BB_ER },
		{ "STATUS_ADDR_ER", STATUS_ADDR_ER },
		{ "STATUS_INT_CP", STATUS_INT_CP },
		{ "STATUS_PIN_BY_N", STATUS_PIN_BY_N },
		{ "STATUS_PIN_WP_N", STATUS_PIN_WP_N },
		{ "STATUS_ILL_LOG", STATUS_ILL_LOG },

	};

	for (int i = 0; i < 10; ++i)
	{
		if (Status & Things[i].Mask)
		{
			printf("%s |", Things[i].Name);
		}
	}
...
void ReadPage(HANDLE Sidecar, DWORD Address, BYTE* Buffer, BOOL Logical)
{
	...
	for (int i = 0; i < GetPageSize( Logical ); i += 4)
	{
...
void ReadPage(HANDLE Sidecar, DWORD BlockIndex, DWORD PageIndex, BYTE* Buffer, BOOL Logical)
{
	...
	ReadPage(Sidecar, Address, Buffer, Logical);
}

void ReadBlock(HANDLE Sidecar, DWORD BlockID, BYTE* Buffer, BOOL Logical)
{
	for (int i = 0; i < GetPagesPerBlock(Logical); ++i)
	{
		ReadPage(Sidecar, BlockID, i, &Buffer[ GetPageSize( Logical ) * i ], Logical);
	}
}

void WritePage(HANDLE Sidecar, DWORD Address, BYTE* Buffer, BOOL Logical)
{
	DWORD Config = ReadSPIReg(Sidecar, SFCX_CONFIG);

	WriteSPIReg(Sidecar, SFCX_CONFIG, Config | CONFIG_WP_EN);
...
void WritePage(HANDLE Sidecar, DWORD BlockIndex, DWORD PageIndex, BYTE* Buffer, BOOL Logical)
{
	...
	auto Address = IndexToAddress(BlockIndex, PageIndex, Logical);

	WritePage(Sidecar, Address, Buffer, Logical);
}

void EraseBlock(HANDLE Sidecar, DWORD BlockID)
{
	DWORD Config = ReadSPIReg(Sidecar, SFCX_CONFIG);
	...
	SPIDoCommand(Sidecar, BLOCK_ERASE);
}

void WriteBlock(HANDLE Sidecar, DWORD BlockID, BYTE * Buffer, BOOL Logical)
{
	...
	for (int i = 0; i < GetPagesPerBlock(Logical); ++i)
	{
		...
		WritePage(Sidecar, BlockID, i, &Buffer[Offset], Logical);
	}
}
...
void FlashNand(HANDLE Sidecar)
{
	...
	if (fopen_s(&FlashFile, "NandToFlash.bin", "rb+") == 0)
	{
		...
		for (int i = 4; i < NumBlocks; i++)
		{
			...
			memset(BlockBuffer, 0x33, JASPER_PHYSICAL_BLOCK_SIZE);

			fseek(FlashFile, i * JASPER_PHYSICAL_BLOCK_SIZE, SEEK_SET);
...
// Yo nigga its ya boi
//imod 1998 is like a good
//yo wtf bro this is mad
